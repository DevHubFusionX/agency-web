      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <motion.div
            ...
            className="lg:col-span-7 bg-white rounded-[3rem] overflow-hidden border border-gray-100 shadow-2xl shadow-gray-100 group"
          >
            <div className="grid grid-cols-1 md:grid-cols-2 h-full">
              ...
              <div className="p-12 flex flex-col justify-center">
                <div className="text-sm font-black text-blue-600 uppercase tracking-[0.2em] mb-4">Lead Visionary</div>
                <h3 className="text-4xl font-black text-gray-900 mb-2">{team[0].name}</h3>
                <p className="text-gray-500 text-lg font-medium mb-6">{team[0].role}</p>
                <p className="text-gray-500 text-lg leading-relaxed mb-8">{team[0].bio}</p>
                <div className="flex flex-wrap gap-2 mb-8">
                  {team[0].expertise.map((skill) => (
                    <span key={skill} className="px-4 py-1.5 bg-gray-50 text-gray-900 text-xs font-bold rounded-full border border-gray-100">
                      {skill}
                    </span>
                  ))}
                </div>
                <div className="flex gap-4">
                  <a href="#" className="w-12 h-12 bg-white border border-gray-100 shadow-sm hover:bg-blue-600 hover:border-blue-600 rounded-2xl flex items-center justify-center transition-all duration-300 group/icon">
                    <Linkedin size={20} className="text-gray-400 group-hover/icon:text-white" />
                  </a>
                  <a href="#" className="w-12 h-12 bg-white border border-gray-100 shadow-sm hover:bg-blue-600 hover:border-blue-600 rounded-2xl flex items-center justify-center transition-all duration-300 group/icon">
                    <Twitter size={20} className="text-gray-400 group-hover/icon:text-white" />
                  </a>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-5 flex flex-col gap-12">
            {team.slice(1).map((member, index) => (
              <motion.div
                ...
                className="bg-white rounded-[2.5rem] p-8 shadow-xl shadow-gray-100 border border-gray-100 hover:shadow-2xl transition-all duration-500 flex gap-8 items-center group"
              >
                <div className="w-32 h-32 rounded-3xl overflow-hidden flex-shrink-0 border-4 border-gray-50">
                  <img src={member.image} alt={member.name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="text-2xl font-black text-gray-900 mb-1">{member.name}</h3>
                  <p className="text-blue-600 text-sm font-bold uppercase tracking-wider mb-4">{member.role}</p>
                  <p className="text-gray-500 leading-relaxed mb-6 line-clamp-2">{member.bio}</p>
                  <div className="flex flex-wrap gap-2">
                    {member.expertise.map((skill) => (
                      <span key={skill} className="px-3 py-1 bg-gray-50 text-gray-500 text-xs font-bold rounded-full">
                        {skill}
                      </span>
                    ))}
                  </div>
